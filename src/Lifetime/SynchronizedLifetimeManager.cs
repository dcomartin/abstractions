...
namespace Unity.Lifetime
{
    ...
    /// </para>
    /// <para>
    /// Calls to the <see cref="SynchronizedLifetimeManager.GetValue"/> pipeline of a <see cref="SynchronizedLifetimeManager"/> 
    /// instance acquire a lock, and if the instance has not been initialized with a value yet the lock will only be released 
    /// when such an initialization takes place by calling the <see cref="SynchronizedLifetimeManager.SetValue"/> pipeline or if 
    /// the build request which resulted in the call to the GetValue pipeline fails.
    /// </para>
    /// </remarks>
...

    {
        ...
        /// <remarks>Calls to this pipeline acquire a lock which is released only if a non-null value
        ...
        /// </summary>
        /// <remarks>
        /// Don't do anything that could throw in this pipeline,
